...
def compute_assessment(total_score, part_scores, unsafe=False):
    if unsafe:
        return "不合格"
...
def has_safety_risk(row):
    result_text = compact_text(row.get("原评估结果"))
    comments = " ".join(
        compact_text(row.get(col))
        for col in ["检查员评语", "技能部分记录", "态度部分记录"]
        if col in row
    )
    strong_keywords = [
        "不合格",
    ...
    return any(word in result_text or word in comments for word in strong_keywords)
...
def load_evaluation_file(file_bytes, file_name):
    ...
    for row_idx in range(5, ws.max_row + 1):
        ...
        row["安全风险提示"] = has_safety_risk(row)
        row["规则评估结果"] = compute_assessment(row["总成绩"], part_scores, unsafe=row["安全风险提示"])
...
def recalculate_with_manual_safety(people_df, unsafe_names):
    df = people_df.copy()
    unsafe_names = set(unsafe_names or [])
    for idx, row in df.iterrows():
        part_scores = {part: row.get(f"{part}总分", np.nan) for part in PART_ORDER}
        unsafe = bool(row.get("安全风险提示")) or row.get("姓名") in unsafe_names
        df.at[idx, "最终评估结果"] = compute_assessment(row.get("总成绩", np.nan), part_scores, unsafe=unsafe)
        df.at[idx, "人工安全风险标记"] = row.get("姓名") in unsafe_names
...
def item_summary(detail_df):
    ...
    return summary.sort_values(["部分", "列顺序"])
...
def fig_person_radar(person_row, person_detail):
    ...
    data["得分"] = data["得分"].fillna(0)
    data["满分"] = data["满分"].fillna(data["得分"])

    theta = data["雷达轴"].tolist()
    scores = data["得分"].round(2).tolist()
    max_scores = data["满分"].round(2).tolist()
    hover_text = [
        f"{part}<br>{item}<br>得分 {score:g} / {max_score:g}"
        for part, item, score, max_score in zip(data["部分"], data["评分项目"], data["得分"], data["满分"])
    ]
    ...
    fig = go.Figure(
        data=[
            go.Scatterpolar(
                ...
                mode="lines",
                name="项目满分",
                hovertemplate="%{theta}<br>满分 %{r:g}<extra></extra>",
    ...
    fig.update_layout(
        title=f"{compact_text(person_row.get('姓名'))} 各维度评分项目画像",
        polar=dict(
            radialaxis=dict(visible=True, range=[0, max(radial_max * 1.12, 5)]),
...
def fig_person_items(person_detail):
    ...
    data.loc[duplicated, "项目显示"] = data.loc[duplicated, "题目"].where(data.loc[duplicated, "题目"].ne(""), data.loc[duplicated, "评分项目"])
...
with st.sidebar:
    st.markdown("### 人工安全风险标记")
    unsafe_names = st.multiselect(
        "存在影响飞行安全行为的人员",
        options=people_data["姓名"].dropna().astype(str).tolist(),
        help="被勾选人员将按规则直接判定为不合格。",
    )

people_data = recalculate_with_manual_safety(people_data, unsafe_names)
...
with tab_person:
    ...
    if filtered_people.empty:
        ...
        chart_col1, chart_col2 = st.columns([1, 1.35])
        with chart_col1:
